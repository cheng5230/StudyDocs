...
uint8_t VoiceSpeed[10]={80,70,60,50,40,30,20,10,5,3};
...
VOID light_key_fun_scene_change (VOID)
{
  ...
  if (key_trig_num < SCENE_UNIT_NUM_MAX)
  {
    ...
    light_dp_data_autosave();
    /*红外按键不处理按键发射*/
    if(light_IrCrtl.IR_INCFlag==0)
    {
      key_trig_num ++;
      PR_NOTICE("my--- %d ",key_trig_num);
    }
    light_IrCrtl.IR_INCFlag = 0;
  }
  else
  {
