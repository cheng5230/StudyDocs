...
uint32_t voiceColor[6]={RED,ORANGE,YELLOW,GREEN,PINK,PURPLE};
uint32_t PaomaColor[3]={0xFF1010,0x10ff10,0x1010ff};//{RED,GREEN,BLUE};
...
void light_VoiceData(uint8_t delta)
{
  ...
  for(i = 1;i < DISPLAY_BUF_MAX_PIXEL;i++)
  {
    __back_buf[i*3+0] = 0;
    __back_buf[i*3+1] = 0;
    __back_buf[i*3+2] = 0;
...
static void __light_scene_WS2812Fx(uint8_t type, uint8_t scene_num)   
{
    ...
    ws2812_timer_handler(type,times,scene_num);
...
STATIC VOID light_switch_set (BOOL stat)
{
  ...
  if (stat == FALSE)
  {
    ...
    light_scene_stop();
    light_shade_stop();
    //PR_NOTICE ("light_switch_set-----------------------=%d--%d-%d",dp_data.mode,light_handle.scene_type,_def_cfg.power_onoff_type);
    memset(&light_handle.fin, 0, sizeof (BRIGHT_DATA_S));
