...
STATIC VOID __userIrCmdDeal(IRCMD cmd, IRCODE irType)
{
  ...
  static uint8_t I = 0;
  static uint8_t rgb_turn_round_cnt = 0;
  static uint8_t cw_turn_round_cnt = 0;
  ...
  switch (last_cmd)
  {
    ...
    case KEY_RGB_TURN_ROUND:
  	  rgb_turn_round_cnt++;
  	  if(rgb_turn_round_cnt%3 == 1){
  	    light_ir_key_fun_color (0);
  	  }
  	  else if(rgb_turn_round_cnt%3 == 2){
  	    light_ir_key_fun_color (120);
  	  }
  	  else if(rgb_turn_round_cnt%3 == 0){
  	    light_ir_key_fun_color (240);
  	    rgb_turn_round_cnt = 0;
  	  }
      break;
	case KEY_CW_TURN_ROUND:
  	  cw_turn_round_cnt++;
  	  if(cw_turn_round_cnt%3 == 1){
  	    light_ir_key_fun_white_w();
  	  }
  	  else if(cw_turn_round_cnt%3 == 2){
  	    light_ir_key_fun_white_nw();
  	  }
  	  else if(cw_turn_round_cnt%3 == 0){
  	    light_ir_key_fun_white_c();
  	    cw_turn_round_cnt = 0;
  	  }
  	  break;
