...
static uint8_t staticFlag,breathflag,raindropflag,colorfulflag,marqueeflag,blinkingflag,snowcolorflag,streamerflag;
  ...
  switch (dp_data.mode)
  {
    ...
    case COLOR_MODE:
      if(data_testFlag)
      {
        ...
        PR_NOTICE("light_handle.fin -> r = %d   g = %d  b = %d w = %d", light_handle.fin.red, light_handle.fin.green,light_handle.fin.blue,light_handle.fin.white);
...
static void __light_scene_Streamer(unsigned char scene_num)
{
  ...
  for(index=0; index < DISPLAY_BUF_MAX_PIXEL; index++) 
  {
      if(index>255)
      {
        color = color_wheel((((index-255) * 256 / DISPLAY_BUF_MAX_PIXEL) + counter_mode_step) & 0xFF);
      }
      else
      {
         color = color_wheel(((index * 256 / DISPLAY_BUF_MAX_PIXEL) + counter_mode_step) & 0xFF);
      }
...
static void __light_scene_flame(unsigned char scene_num)
{
  ...
  light_scene_data_getPara(&(get_dp_data()->scene[SCENE_HEAD_LEN+__scene_cnt*SCENE_UNIT_LEN]), SCENE_TYPE_RAIN_DROP, &times, &speed);
    ...
    uint8_t w = (0xFF0000 >> 24) & 0xFF;
    uint8_t r = (0xFF0000 >> 16) & 0xFF;
    uint8_t g = (0xFF0000>>  8) & 0xFF;
    uint8_t b = (0xFF0000       & 0xFF);
    uint8_t lum = max(w, max(r, max(g, b))) / 12;
    ...
    for(index=0; index <= DISPLAY_BUF_MAX_PIXEL; index++) {
      ...
      b =  max(b - flicker, 0);
       ...
       get_light_fin()->r = b;
		   get_light_fin()->g = g>>8;
		   get_light_fin()->b = r>>8;
       update_display_one_pixel(get_light_fin()->r, get_light_fin()->g, get_light_fin()->b, get_light_fin()->c, get_light_fin()->w, index); 
    }
    #endif
    /*********** 时间调节控制 *******/
    //(速度---65-5)
...
void nscale8(uint16_t num_leds, uint8_t scale)
{
    uint8_t r,g,b;
    uint16_t i;
    
    for(i = 0; i < num_leds; i++) {
		    r =((uint32_t)__back_buf[i*3]* (uint32_t)(scale))>>8;
        g =((uint32_t)__back_buf[i*3+1]* (uint32_t)(scale))>>8;
        b =((uint32_t)__back_buf[i*3+2] * (uint32_t)(scale))>>8;

        get_light_fin()->red=r;
    		get_light_fin()->green=g;
    		get_light_fin()->blue=b;
        update_display_one_pixel(get_light_fin()->red, get_light_fin()->green, get_light_fin()->blue, get_light_fin()->white, get_light_fin()->warm, i);
    }
}

void fadeToBlackBy(  uint16_t num_leds, uint8_t fadeBy)
{
    nscale8(num_leds, fadeBy);
}
...
static void __light_scene_confetti(unsigned char scene_num)   
{
    ...
    #if 0
    /* 满天星 效果 */
    fadeToBlackBy(DISPLAY_BUF_MAX_PIXEL, 0);
    pos = random8Var(DISPLAY_BUF_MAX_PIXEL);
    get_light_fin()->r +=random8Var(120);
    get_light_fin()->g +=random8Var(120);//200;
    get_light_fin()->b +=random8Var(120);//250;
    update_display_one_pixel(get_light_fin()->r, get_light_fin()->g, get_light_fin()->b, get_light_fin()->c, get_light_fin()->w, pos);
    #else
    /* 彩虹流动效果 */
    tricolor_chase(RED,GREEN,BLUE);
    #endif
     /*********** 时间调节控制 *******/
    /*** (速度---65-5) ********/
 ...
 */
 ...
 static void __light_scene_SportLight(unsigned char scene_num)   
 {
    ...
    WS2812FX_modeflow(&trip_times[1],DISPLAY_BUF_MAX_PIXEL/TRIPLEDS_DIV,YELLOW);
    ...
    WS2812FX_modeflow(&trip_times[4],DISPLAY_BUF_MAX_PIXEL/TRIPLEDS_DIV+DISPLAY_BUF_MAX_PIXEL/TRIPLEDS_DIV+DISPLAY_BUF_MAX_PIXEL/TRIPLEDS_DIV+DISPLAY_BUF_MAX_PIXEL/TRIPLEDS_DIV,PURPLE);
    WS2812FX_modeflow(&trip_times[5],DISPLAY_BUF_MAX_PIXEL,PINK);
    /*********** 时间调节控制 *******/
    /*** (速度---65-5) ********/
...
VOID light_scene_start (VOID)
{
  ...
  /********************************************************************/
  __start_scene_timer(BASESCENE_TIMES*10);
   __scene_cnt = 0;
   __marquee_fun_value = 0;
   __multi_fun_value = 0;  
   __marquee_cnt0 = 0;
...
STATIC VOID light_switch_set (BOOL stat)
{
  ...
  if (stat == FALSE)
  {
    ...
    light_scene_stop();
    light_shade_stop();
    PR_DEBUG ("light_switch_set-----------------------=%d--%d-%d",dp_data.mode,light_handle.scene_type,_def_cfg.power_onoff_type);
    memset(&light_handle.fin, 0, sizeof (BRIGHT_DATA_S));
...
STATIC OPERATE_RET light_thread_init (VOID)
{
  ...
  SystemSleep (10);
    op_ret = tuya_light_CreateAndStart (&thread_scenecd, light_thread_sceneCb, NULL, 2048, TRD_PRIO_2, "flash_scene_task");
