...
void wifi_uart_service(void)
{
  ...
  if(rx_in < rx_value_len)
  {
      ...
      else
      {
        ...
        if(mcu_VoiceTemp!=deltavoice)
        {
           ...
           {
              __voice_cnt++;
              mcu_VoiceSPK += mcu_VoiceTemp;
              if(__voice_cnt==3)
              {
                 __voice_cnt = 0;
                 //mcu_VoiceSPK = mcu_VoiceSPK/3;
                 /****** 音量大小10个等级 **********/
                 mcu_VoiceTicks = ((mcu_VoiceSPK/100)>9)?9:(mcu_VoiceSPK/100);
                 mcu_VoiceSpeed = VoiceSpeed[mcu_VoiceTicks];
                 //PR_NOTICE("11111111111-- %d %d",mcu_VoiceSpeed,mcu_VoiceTicks);
              }
           }
        }
        else
             ...
             if((mcu_times%100)==0) 
             {
                if(mcu_VoiceSPK)
                {
                    ...
                    if(__voice_cnt==3)
                    {
                       ...
                       mcu_VoiceSPK = mcu_VoiceSPK/3;
                       /****** 音量大小10个等级 **********/
                       mcu_VoiceTicks = ((mcu_VoiceSPK/50)>9)?9:(mcu_VoiceSPK/50);
                       mcu_VoiceSpeed = VoiceSpeed[mcu_VoiceTicks];
                       //PR_NOTICE("00000000000-- %d %d",mcu_VoiceSpeed,mcu_VoiceTicks);
                    }
                }
             }
          }
    ...
    &&(wifi_uart_rx_buf[3] == 0x02)&&(wifi_uart_rx_buf[6] == 0x45)&&(wifi_uart_rx_buf[7] == 0x44))
  {
      ...
      if(len!=mcu_VoiceData)
      {
          ...
          { 
            deltavoice = mcu_VoiceData-128;
            mcu_VoiceTicks = deltavoice;
          }
          else
          {
            deltavoice = 128 - mcu_VoiceData;
            mcu_VoiceTicks = 0;
          }
          ...
          if(_light_threadFlag==false)
          {
             ...
             deltavoice = 128 - mcu_VoiceData;
             //PR_NOTICE("---------------- %d",deltavoice);
             light_VoiceData(deltavoice);       
            /******************/
          }
          #endif
      }
