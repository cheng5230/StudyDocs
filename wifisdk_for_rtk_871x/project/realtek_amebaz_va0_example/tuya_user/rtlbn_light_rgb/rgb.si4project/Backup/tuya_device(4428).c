...
STATIC VOID __userIrCmdDeal (IRCMD cmd, IRCODE irType)
{
  ...
  {
     if((last_cmd!=KEY_B_LEVEL_4)||(last_cmd!=KEY_R_LEVEL_2)||(last_cmd!=KEY_W_LEVEL_5)||(last_cmd!=KEY_MODE_STROBE)||(last_cmd!=KEY_B_LEVEL_3)||(last_cmd!=KEY_B_LEVEL_5)||(last_cmd!=KEY_MODE_FLASH))
     {
   ...
   /********  手动模式处理 **************/
    ...
    case KEY_G_LEVEL_4:
        ...
        if(UserIndex>=light_IrCrtl.ModeTotals)
        {
            ...
            if(key_trig_num==SCENE_TYPE_RAIN_DROP)
            {
               light_IrCrtl.ModeTotals = FIRE_MODES; 
            }
            if(key_trig_num==SCENE_TYPE_COLORFULL)
            {
            ...
            PR_NOTICE("index0 ++++ %d %d",key_trig_num,UserIndex);
            light_ir_fun_scene_change(key_trig_num);
            PR_NOTICE("index1 ++++ %d %d",key_trig_num,UserIndex);
        }
      break;
    // 手动模式功能减
    case KEY_MODE_FLASH:
        ...
        PR_NOTICE("index ---- %d %d",key_trig_num,UserIndex);
      break;
    ...
    case KEY_B_LEVEL_5:
      ...
      if(key_trig_num<4)
      {
        if(light_IrCrtl.Speedtimes<99)
        light_IrCrtl.Speedtimes = light_IrCrtl.Speedtimes+2;
        else
