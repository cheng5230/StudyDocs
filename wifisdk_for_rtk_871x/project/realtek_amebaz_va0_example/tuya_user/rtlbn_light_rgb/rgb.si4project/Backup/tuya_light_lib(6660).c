...
static void __light_music_WS2812Fx(uint8_t type, uint8_t scene_num)  
{
    ...
    if(dpMusicCnts)
    {
      ...
      {
         if((dpMusicticks%500)==0)
         {
            dpMusicCnts--;
         }
      }
      PR_NOTICE("++++++++++%d %d %d",(dpMusicticks%500),dpMusicticks,dpMusicCnts);
    }

    /********音乐最大数值限定在50********/
    dpMusicvoice = 19+dpMusicCnts;
    mcu_VoiceTicks = dpMusicvoice+dpMusicCnts<<1;
