...
extern void send_light_data(unsigned int gdata, unsigned int rdata, unsigned int bdata, unsigned int cdata, unsigned int wdata);
...
{
  send_light_data(G_value,R_value,B_value,CW_value, WW_value);
  //tuya_light_send_pwm_data (R_value, G_value, B_value, CW_value, WW_value);
}

VOID tuya_prod_light_send_data (IN USHORT_T R_value, IN  USHORT_T G_value, IN  USHORT_T B_value, IN USHORT_T CW_value, IN USHORT_T WW_value)
{
...
STATIC VOID set_wf_gw_status (IN CONST GW_WIFI_NW_STAT_E stat)
{
  ...
  tuya_light_wf_stat_cb (wifi_stat); /* 根据wifi状态来处理--显示式样�?*/
  return;
}
...
VOID tuya_light_dev_reset (VOID)
{
  ...
  tuya_iot_wf_gw_fast_unactive (light_get_wifi_cfg(), WF_START_SMART_ONLY);
}
