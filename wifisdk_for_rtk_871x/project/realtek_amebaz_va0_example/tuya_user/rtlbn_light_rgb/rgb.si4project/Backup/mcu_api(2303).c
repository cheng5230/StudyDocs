...
unsigned int  g_musicvoice;
unsigned char g_dpmode;
...
/*** 音量节奏控制七彩变化* ***/
/*
*  mcu_VoiceTemp ---- 控制亮度 -- 最大输入数值50
*  mcu_VoiceTicks --- 控制节拍 -- 数值波动变化
*/
void VoiceTricolor_mode(unsigned int voicedata,unsigned char speed)
{
    /******** 声音7彩切换效果 *******/
    if(g_dpmode)
    {
    ...
    {
       light_VoiceData(voicedata,speed);
    }
    /**********************/
}

/** 音量大小控制节奏 *********/
void VoiceSpk_mode(unsigned int voicedata,unsigned char speed)
{
    unsigned char temp;
    /**************** 声控音量速度        *************/
    if(g_dpmode)
    ...
    else
    {
        if(speed)
        {
          mcu_VoiceSpeed = speed;
         /************* 亮度控制 ****************/
         if(voicedata<20)
          temp = 20;
         else if(voicedata<51)
          temp = voicedata;
         else
          temp = 50;
...
/** 音量控制亮度，节奏控制大小 **/
void Voicelum_mode(unsigned int voicedata,unsigned char speed)
{
    unsigned char temp;
...
void MyVocie_SelectMode(void)
{
   ...
   {
      micspeed = mcu_VoiceTicks;
      micdata = g_musicvoice;
   }
   PR_NOTICE("---%d %d %d",dpMusictimes,micspeed,micdata);
   /******* 音乐模式处理 **************/
   if(g_scenemode == SCENE_TYPE_MARQUEE)
   {
      VoiceTricolor_mode(micdata,micspeed);
   }
   else if(g_scenemode == SCENE_TYPE_BLINKING)
   {
      VoiceSpk_mode(micdata,micspeed);
   }
   else 
   {
      Voicelum_mode(micdata,micspeed);
   }
}
