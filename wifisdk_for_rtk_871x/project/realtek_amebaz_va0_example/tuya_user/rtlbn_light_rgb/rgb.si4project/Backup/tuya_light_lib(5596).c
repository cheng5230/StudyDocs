...
void light_scene_SetDataPara(unsigned char times, unsigned char speed)
{
  ...
  PR_NOTICE("------------------------------------------------------------- %s  %d %d %d %d", dp_data.scene,__scene_cnt,times,speed,mydata);
  light_color_data_hex2asc (&(get_dp_data()->scene[SCENE_HEAD_LEN+(__scene_cnt)*SCENE_UNIT_LEN]), mydata, 4);
  PR_NOTICE("------------------------------------------------------------- %s", dp_data.scene);
  light_bright_start();
...
void IR_AutomMode(void)
{
  ...
  {
    PR_NOTICE("-------%d ",light_IrCrtl.IR_Runtimes);
    if(light_IrCrtl.IR_Runtimes++>10000) //IRAUTO_RUNTIMES
    {
