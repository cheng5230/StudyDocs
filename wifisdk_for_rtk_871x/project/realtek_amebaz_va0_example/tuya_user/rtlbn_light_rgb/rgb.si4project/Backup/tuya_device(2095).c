...
STATIC VOID __userIrCmdDeal (IRCMD cmd, IRCODE irType)
{
  ...
  #if USER_IRREMOTE
  /***** 红外自动运行模式处理 ***/
  if(last_cmd!=KEY_B_LEVEL_4)
  {
    if(light_IrCrtl.IR_Mode==1)
    {
      if(light_IrCrtl.IRFlag==0)
      {
  ...
  #if USER_IRREMOTE
  {
   ...
   /********  手动模式处理 **************/
    ...
    case KEY_MODE_FLASH:
     ...
     // 速度加
    case KEY_B_LEVEL_5:
      PR_NOTICE("+++++++++ num %d %d",key_trig_num,light_IrCrtl.Speedtimes);
      if(key_trig_num<4)
      {
        if(light_IrCrtl.Speedtimes>5)
        light_IrCrtl.Speedtimes = light_IrCrtl.Speedtimes-2;
        else
          light_IrCrtl.Speedtimes = 5;
   ...
   case KEY_B_LEVEL_3:
      ...
      if(key_trig_num<4)
      {
        if(light_IrCrtl.Speedtimes<65)
        light_IrCrtl.Speedtimes = light_IrCrtl.Speedtimes+2;
        else
          light_IrCrtl.Speedtimes = 65;
        light_IrCrtl.SpeedFlag = 1;
        light_scene_SetDataPara(light_IrCrtl.Speedtimes,light_IrCrtl.Speedtimes);
      }
      break;  
   ...
   case KEY_B_LEVEL_4:
      if(light_IrCrtl.IR_Mode==0)
      {
        ...
        light_IrCrtl.IR_Cnts = 0;
        light_IrCrtl.IR_Runtimes = 0;
        light_ir_fun_scene_change(light_IrCrtl.IR_Cnts);
      }
      break;
