...
void gtimer_reload (gtimer_t *obj, uint32_t duration_us)
{
	uint32_t tid = obj->timer_id;
        uint32_t temp;
	if(tid<TIMER4)
	 temp = (uint32_t)((float)duration_us  / 1000000 *32768);  // 32.768khz
    else
	 temp = 1;//(uint32_t)((float)duration_us / 25); // 40M = 40000000
	 DBG_8195A("id=%d,tmp=%d\n",tid,temp);  // id=4,tmp=4
	assert_param((tid < GTIMER_MAX) && (tid > TIMER0));
