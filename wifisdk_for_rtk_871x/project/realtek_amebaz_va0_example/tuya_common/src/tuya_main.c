...
*************************micro define***********************
***********************************************************/
#define TEST_SSID "tuya_mdev_test00"
...
STATIC OPERATE_RET __gw_upgrage_process_cb(IN CONST FW_UG_S *fw, IN CONST UINT total_len,IN CONST UINT offset,IN CONST BYTE_T *data,IN CONST UINT len,OUT UINT *remain_len, IN PVOID pri_data);
...
STATIC OPERATE_RET __gw_upgrage_process_cb(IN CONST FW_UG_S *fw, IN CONST UINT total_len,IN CONST UINT offset, IN CONST BYTE_T *data,IN CONST UINT len,OUT UINT *remain_len, IN PVOID pri_data)
{
    ...
    switch(ug_proc->stat) {
        ...
        case UGS_RECV_OTA_HD: {
            ...
            if(new_img_len > (468*1024 -32)) 
            {
              //32 Bytes resvd for header 468K
            ...
            if(new_img_len > (788*1024 - 32)) 
            {
                //32 Bytes resvd for header 960K
                PR_ERR("image length is too big");
                return OPRT_COM_ERROR;
            }
            #endif
