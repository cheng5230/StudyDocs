...
STATIC VOID __userIrCmdDeal (IRCMD cmd, IRCODE irType)
{
  ...
  PR_NOTICE("-------------%d %d %x",light_IrCrtl.IR_Mode,light_IrCrtl.IR_Flag,last_cmd);
  if(light_IrCrtl.IR_Mode)
  {
    if(light_IrCrtl.IR_Flag==0)
    {
       ...
       {
          /**自动模式下，模式加减无效果**/
          PR_NOTICE("0000000000000 %x",last_cmd);
       ...
       {
          PR_NOTICE("1111111111 %x",last_cmd);
          light_IrCrtl.IR_Mode = 0;
       }
    }
  }
  ...
  #if USER_IRREMOTE
  {
   ...
   /********  手动模式处理 **************/
    ...
    case KEY_G_LEVEL_4:
        ...
        __mode_cnts = 0;
        UserIndex++;
        //PR_NOTICE("index0 ++++ %d %d",light_IrCrtl.ModeTotals,UserIndex);
