...
/*** 音量节奏控制七彩变化* ***/
void VoiceTricolor_mode(unsigned int voicedata)
{
    /******** 声音7彩切换效果 *******/
    /**** 声音测试 *****/
    if(g_dpmode)
    ...
    if(mcu_VoiceTemp!=voicedata)
    {
      ...
      PR_NOTICE("+++++++++++++++++%d %d %d %d %d",dpMusicticks,dpMusicType,dpMusicCnts,mcu_VoiceTemp,mcu_VoiceTicks);
      /*
...
/** 音量大小控制节奏 *********/
void VoiceSpk_mode(unsigned int voicedata)
{
   /**************** 声控音量速度        *************/
    if(mcu_VoiceTemp!=voicedata)
    {
...
/** 音量控制亮度，节奏控制大小 **/
void Voicelum_mode(unsigned int voicedata)
{
    unsigned char temp;
...
void MyVocie_SelectMode(void)
{
   ...
   {
      VoiceTricolor_mode(micdata);
   }
   else if(g_scenemode == SCENE_TYPE_BLINKING)
   {
      VoiceSpk_mode(micdata);
   }
   else 
   {
      Voicelum_mode(micdata);
   }
}
