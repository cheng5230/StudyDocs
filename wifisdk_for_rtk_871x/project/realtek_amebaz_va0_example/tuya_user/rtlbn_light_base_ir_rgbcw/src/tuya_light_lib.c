  ...
  while (1)
  {
    ...
    if (memcmp (&light_handle.fin, &light_handle.curr, SIZEOF (BRIGHT_DATA_S) ) != 0)
    {
      ...
      if (dp_data.power == FALSE) 
      {
        ...
        light_send_data (light_handle.curr.red, light_handle.curr.green, light_handle.curr.blue, light_handle.curr.white,
                         light_handle.curr.warm);
      }
    }
    else
    {
      //结束变化,显示一次目标值
      light_shade_stop();
      PR_DEBUG ("light send data %d %d %d %d %d", light_handle.curr.red, light_handle.curr.green, light_handle.curr.blue,
                light_handle.curr.white, light_handle.curr.warm);
...
VOID light_ir_fun_power_on_off_ctrl (VOID)
{
#if USER_DEFINE_LOWER_POWER
...
VOID light_ir_key_fun_white_xiaoye()
{
  if (dp_data.power != TRUE)
  {
    return;
...
VOID light_ir_key_fun_white_nw()
{
  if (dp_data.power != TRUE)
  {
    return;
